...
export class Visual implements IVisual {
    ...
    container.style.setProperty("font-size", `${this.fontSize}px`, "important");
    container.style.lineHeight = "1.3em";
    container.innerHTML = marked.parse(text || "") as string;

    // Botão de impressão
    const printButton = document.createElement("button");
    printButton.innerText = "Salvar como PDF";
    ...
    if (type === "status") {
        ...
        this.statusOutput.appendChild(container);
    } else if (type === "result") {
        this.resultOutput.appendChild(container);
    } else if (type === "debug" && this.debug === "true") {
        this.debugOutput.appendChild(container);
    }
}
